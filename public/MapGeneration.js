...
export class MapGenerator {
  ...
  /**
   * Initialize asset management by creating or reusing ShardManager instance
  ...
  initAssetManagement() {
    ...
    try {
      // First check if there's already a global ShardManager instance to reuse
      if (window.shardManager) {
        console.log('[MAP] Using existing global ShardManager instance');
        this.shardManager = window.shardManager;
        
        // Verify the shardManager has all required dependencies
        const hasRequiredDeps = (
          this.shardManager.THREE && 
          this.shardManager.scene && 
          this.shardManager.assetLoader
        );
        
        if (hasRequiredDeps) {
          console.log('[MAP] Global ShardManager has all required dependencies');
          return;
        } else {
          console.warn('[MAP] Global ShardManager missing dependencies, will reinitialize');
          // Continue to create a new instance
        }
      }
      
      // Check for global AssetLoader
      const assetLoader = window.assetLoader;
      if (!assetLoader) {
        console.warn('[MAP] No global AssetLoader found. Will attempt other initialization methods.');
      } else {
        console.log('[MAP] Found global AssetLoader instance to use with ShardManager');
      }
      ...
      console.log('[MAP] Creating ShardManager instance with configuration:', {
      ...
      this.shardManager = new window.ShardManager({
        scene: this.scene,
        THREE: this.THREE,
        assetLoader: assetLoader, // Pass global AssetLoader if available
        ...this.config.shardConfig
      });
      
      // Make it globally available for other components
      window.shardManager = this.shardManager;
      
      console.log('[MAP] ShardManager initialized successfully:', {
        hasScene: !!this.shardManager.scene,
        hasTHREE: !!this.shardManager.THREE,
        hasAssetLoader: !!this.shardManager.assetLoader,
        isGlobal: window.shardManager === this.shardManager
      });
    } catch (error) {
      ...
      // Initialize crystal methods to ensure fallback is available
