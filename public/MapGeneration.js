...
function attemptLoadFBXLoader() {
  console.log(
    `[MAP] Attempting to load FBXLoader via ES6 import (attempt ${fbxLoaderAttempts + 1}/${FBXLOADER_PATHS.length})`,
  );

  if (fbxLoaderAttempts >= FBXLOADER_PATHS.length) {
    console.warn(
      "[MAP] All FBXLoader ES6 import attempts failed, trying script tag approach",
    );
  ...
  return import(path)
    .then((module) => {
      console.log(
        "[MAP] Import succeeded, module contents:",
        Object.keys(module),
      );
      ...
      if (module.FBXLoader) {
        ...
        console.log(
          "[MAP] FBXLoader imported successfully via module.FBXLoader",
        );
        ...
        console.log(
          "[MAP] FBXLoader imported successfully via module.default.FBXLoader",
        );
        window.FBXLoader = FBXLoader; // Make it available globally
        fbxLoaderSuccess = true;
        return true;
...
function attemptLoadFBXLoaderViaScript() {
  ...
  console.log(
    `[MAP] Attempting to load FBXLoader via script tag from: ${scriptPath}`,
  );
  ...
  return new Promise((resolve) => {
    ...
    script.onload = () => {
      console.log(
        "[MAP] Script loaded successfully, checking for FBXLoader...",
      );
      ...
      if (typeof window.FBXLoader === "function") {
        ...
        console.warn(
          "[MAP] Script loaded but FBXLoader not found on window or THREE",
        );
        window._fbxLoaderErrors.push({
...
try {
  console.log(
    "[MAP] Starting FBXLoader loading process with enhanced diagnostics",
  );
...
export class MapGenerator {
  constructor(scene, THREE) {
    ...
    this.config = {
      ...
      crystalScaleFactor: 0.005, // Size of the crystal (scale - increased from 0.005 to make crystals more visible)
  ...
  initCrystalMethods() {
    ...
    this.createFallbackCrystal = (hex) => {
      ...
      try {
        ...
        if (!hex || !hex.position) {
          console.error(
            "[MAP] Invalid hex object provided to createFallbackCrystal:",
            hex,
          );
          return null;
        }
        ...
        if (this.THREE.BufferGeometryUtils) {
          console.log(
            "[MAP] Using BufferGeometryUtils to create merged geometry",
          );
          ...
          console.log(
            "[MAP] BufferGeometryUtils not available, using simple geometry",
          );
        ...
        try {
          if (this.THREE.MeshPhysicalMaterial) {
            ...
            console.log(
              "[MAP] Successfully created MeshPhysicalMaterial for crystal",
            );
          } else {
          ...
          console.warn(
            "[MAP] Could not create MeshPhysicalMaterial, using enhanced MeshPhongMaterial:",
        ...
        crystal.position.set(
          ...
          hex.position.y +
            this.config.hexHeight / 2 +
            this.config.crystalHeightOffset +
            randomOffset,
          hex.position.z + (Math.random() - 0.5) * 0.1,
        );
        console.log(
          `[MAP] Positioned crystal at: ${crystal.position.x.toFixed(2)}, ${crystal.position.y.toFixed(2)}, ${crystal.position.z.toFixed(2)}`,
        );
        ...
        console.log(
          `[MAP] Applied crystal Y rotation: ${crystal.rotation.y.toFixed(2)} radians`,
        );
        console.log(
          `[MAP] Applied random rotation: ${crystal.rotation.x.toFixed(2)}, ${crystal.rotation.y.toFixed(2)}, ${crystal.rotation.z.toFixed(2)}`,
        );
        ...
        console.log(
          `[MAP] Enhanced fallback crystal placed on hex (${hex.userData.q}, ${hex.userData.r})`,
          {
  ...
  async trySpawnCrystalShard(hex) {
    try {
      ...
      console.log(
        `[MAP] Evaluating crystal spawn for hex at (${hex.userData.q}, ${hex.userData.r})`,
      );

      // First validate the hex is a valid object with necessary properties
      if (!hex || !hex.userData || hex.userData.crystal) {
        console.log(
          `[MAP] Skipping crystal spawn - invalid hex or crystal already exists`,
        );
        return;
      }
      ...
      if (!debugForceSpawn && Math.random() >= this.config.crystalSpawnChance) {
        console.log(
          `[MAP] Crystal spawn skipped for hex at (${hex.userData.q}, ${hex.userData.r}) - random check failed`,
        );
        return;
      }

      console.log(
        `[MAP] Spawning crystal shard on hex at (${hex.userData.q}, ${hex.userData.r})`,
      );

      // Initialize the crystal loader if it doesn't exist yet
      const loaderInitialized = await this.initializeCrystalLoader();
      console.log(
        `[MAP] Crystal loader initialization result: ${loaderInitialized ? "SUCCESS" : "FAILED"}`,
      );
      ...
      if (this.crystalLoader) {
        ...
        console.log(
          "[MAP] Using fallback crystal method as loader is not available",
        );
      ...
      try {
        ...
        console.error(
          "[MAP] Even fallback crystal creation failed:",
          fallbackError,
        );
      }
    }
  ...
  async initializeCrystalLoader() {
    ...
    if (!MapGenerator._initializingLoader) {
      ...
      console.log(
        "[MAP] Another initialization is already in progress, waiting...",
      );
    ...
    if (this.crystalLoader) {
      console.log(
        "[MAP] Crystal loader already initialized from:",
    ...
    console.log(
      "[MAP] Crystal loader not initialized yet, checking available loaders",
    );
    ...
    try {
      ...
      if (fbxLoaderPromise && !fbxLoaderSuccess) {
        ...
        try {
          ...
          console.log(
            "[MAP] FBXLoader import completed, success status:",
            fbxLoaderSuccess,
          );
        } catch (err) {
      ...
      await new Promise((resolve) => {
        ...
        script.onload = () => {
          console.log(
            "[MAP] Direct script load succeeded, checking for FBXLoader...",
          );
          resolve();
        };
        script.onerror = (e) => {
      ...
      console.warn(
        "[MAP] FBXLoader not available! Will use fallback crystals.",
        {
  ...
  ensureFallbackCrystalMethod() {
    ...
    console.error(
      "[MAP] Critical error: createFallbackCrystal is not defined!",
    );
    ...
    this.createFallbackCrystal = (hex) => {
      console.log(
        "[MAP] Using emergency inline fallback crystal creation for hex:",
        {
          position: hex?.position
            ? [hex.position.x, hex.position.y, hex.position.z]
            : "undefined",
      ...
      try {
        ...
        if (!hex || !hex.position) {
          console.error(
            "[MAP] Invalid hex provided to emergency fallback crystal creation",
          );
          return null;
        }
        ...
        console.log(
          `[MAP] Applied emergency crystal Y rotation: ${crystal.rotation.y.toFixed(2)} radians`,
        );
        ...
        console.log(
          `[MAP] Emergency fallback crystal placed on hex (${hex.userData.q || "?"}, ${hex.userData.r || "?"})`,
        );
  ...
  loadCrystalModel(hex) {
    console.log("[MAP] Starting crystal model loading process for hex:", {
      hexPosition: hex.position
        ? [
            hex.position.x.toFixed(2),
            hex.position.y.toFixed(2),
            hex.position.z.toFixed(2),
          ]
        : "undefined",
      hexCoords: hex.userData
        ? `(${hex.userData.q}, ${hex.userData.r})`
        : "unknown",
      ...
      this.crystalLoader.load(
        ...
        (object) => {
          try {
            ...
            console.log("[MAP] Original crystal model size:", {
              ...
              boundingBox: [
                [
                  originalBox.min.x.toFixed(2),
                  originalBox.min.y.toFixed(2),
                  originalBox.min.z.toFixed(2),
                ],
                [
                  originalBox.max.x.toFixed(2),
                  originalBox.max.y.toFixed(2),
                  originalBox.max.z.toFixed(2),
                ],
              ],
            });
            ...
            const adjustedScaleFactor =
              maxDimension > 1000
                ? this.config.crystalScaleFactor / 100
            ...
            console.log(
              `[MAP] Positioned loaded crystal model at: ${object.position.x.toFixed(2)}, ${object.position.y.toFixed(2)}, ${object.position.z.toFixed(2)}`,
            );
            ...
            console.log(
              `[MAP] Applied loaded crystal Y rotation: ${object.rotation.y.toFixed(2)} radians`,
            );

            // Apply enhanced materials and textures to make the crystal look better
            console.log(
              "[MAP] Applying enhanced materials to loaded crystal model",
            );
            ...
            object.traverse((child) => {
              ...
              if (child.isMesh) {
                ...
                try {
                  ...
                  console.log(
                    `[MAP] Original material type: ${originalMaterialType}`,
                  );
                  ...
                  if (this.THREE.MeshPhysicalMaterial) {
                    ...
                    console.log(
                      "[MAP] Applied MeshPhysicalMaterial to crystal model part",
                    );
                  } else {
                    // Create a new MeshBasicMaterial for maximum visibility
                    console.log(
                      "[MAP] Creating high-visibility debug material",
                    );
                    ...
                    console.log(
                      "[MAP] Applied high-visibility MeshBasicMaterial to enhance visibility",
                    );
                  }
                  ...
                  console.error(
                    `[MAP] Error enhancing material for mesh ${child.name || "unnamed"}:`,
                    materialError,
                  );
            ...
            console.log(
              `[MAP] Enhanced materials for ${enhancedMeshCount} meshes in crystal model`,
            );
            ...
            console.log(
              `[MAP] Object added to scene successfully: ${isInScene}`,
              {
            ...
            console.log(
              "[MAP] Materials found in crystal model:",
              materialCounts,
            );
            ...
            console.log(
              `[MAP] Crystal placed on hex at (${hex.userData.q}, ${hex.userData.r})`,
            );
            ...
            if (distance > 1000) {
              console.warn(
                "[MAP] WARNING: Crystal position is very far from origin!",
                {
            ...
            console.error(
              "[MAP] Error processing loaded crystal model:",
              modelError,
            );
        ...
        (xhr) => {
          if (xhr.lengthComputable) {
            ...
            console.log(
              `[MAP] Crystal model ${percentComplete.toFixed(2)}% loaded`,
            );
          }
        },
